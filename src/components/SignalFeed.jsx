...
function SignalCard({ signal }) {
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-sm p-4 ${signal.hot ? 'border-red-200 ring-1 ring-red-100' : 'border-gray-200'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            ...
            <span className="text-xs text-slate-400 ml-auto">{signal.date}</span>
          </div>
          <p className="text-sm font-semibold text-slate-900 mb-1">{signal.title}</p>
          <p className="text-xs text-slate-500 mb-2">{signal.detail}</p>
          <div className="bg-indigo-50 rounded-lg px-3 py-2 border border-indigo-100">
            <p className="text-xs font-semibold text-indigo-500 mb-0.5">Why this matters for ClarityQ</p>
...
export default function SignalFeed() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      {hotSignals.length > 0 && (
        <div className="mb-7">
          <p className="text-xs font-semibold text-red-500 uppercase tracking-widest mb-3">🔥 Hot — Act Now</p>
          <div className="space-y-3">
            {hotSignals.map((signal) => (
              <div key={signal.id}>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-sm">{signal.companyEmoji}</span>
                  <span className="text-xs font-semibold text-slate-600">{signal.companyName}</span>
                </div>
      ...
      <div>
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">All Signals by Company</p>
          ...
          {companies.map((company) => (
            <div key={company.id}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-slate-400">· {company.signals.length} signals</span>
              </div>
