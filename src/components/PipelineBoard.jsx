...
function ProspectCard({ prospect, onLogOutcome }) {
  ...
  const urgencyColor =
    days >= 7 ? 'text-red-500' :
    days >= 3 ? 'text-amber-500' : 'text-gray-400'
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-3 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-sm font-semibold text-gray-900 leading-tight">{prospect.name}</p>
          <p className="text-xs text-gray-500">{prospect.title}</p>
        </div>
        <span className="text-lg shrink-0">{prospect.companyEmoji}</span>
      </div>
      <p className="text-xs font-medium text-gray-600">{prospect.company}</p>
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="bg-white border border-gray-100 rounded-lg px-4 py-2.5 shadow-sm">
          <p className="text-xs text-gray-400">Total Prospects</p>
          <p className="text-2xl font-bold text-gray-900">{total}</p>
        </div>
        <div className="bg-white border border-gray-100 rounded-lg px-4 py-2.5 shadow-sm">
          <p className="text-xs text-gray-400">Meetings Booked</p>
          <p className="text-2xl font-bold text-emerald-600">{booked}</p>
        </div>
        <div className="bg-white border border-gray-100 rounded-lg px-4 py-2.5 shadow-sm">
          <p className="text-xs text-gray-400">Conversion Rate</p>
          <p className="text-2xl font-bold text-indigo-600">
            {total > 0 ? `${Math.round((booked / total) * 100)}%` : '—'}
          </p>
        </div>
      </div>
